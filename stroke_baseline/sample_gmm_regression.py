...
from .gmm_regression_model import GMMCoordDecoderConfig, TextConditionedGMMCoordModel, decode_gmm
from .pretrained_encoder_decoder import DEFAULT_TEXT_ENCODER_DIR
from .regression_dataset import REGRESSION_ID_TO_PEN, StrokeRegressionJsonlDataset
...
def load_model(checkpoint_path: str | Path, device: torch.device, text_encoder_dir: str | None = None):
    ...
    cfg = GMMCoordDecoderConfig(**state["decoder_cfg"])
    enc_dir = text_encoder_dir or state.get("text_encoder_dir", str(DEFAULT_TEXT_ENCODER_DIR))
    model = TextConditionedGMMCoordModel(cfg, text_encoder_dir=enc_dir, max_text_len=state["max_text_len"])
...
def plot_teacher_forced(
    ...
    for xy, true_d, pred_d in zip(coords_abs, true_dxdy, pred_dxdy):
        ...
        axes[0].plot([xy[0], xy[0] + pred_d[0]], [xy[1], xy[1] + pred_d[1]], color="#d62728", linewidth=1.3, alpha=0.8)

    axes[1].set_title("cumulative trajectory")
    axes[1].plot(true_cum[:, 0], true_cum[:, 1], color="0.25", linewidth=2, label="true")
    axes[1].plot(pred_cum[:, 0], pred_cum[:, 1], color="#d62728", linewidth=1.5, label="pred")
...
def strokes_from_deltas(deltas: torch.Tensor, pens: torch.Tensor | None = None) -> list[dict]:
    if pens is None:
        return [{"dx": float(dx), "dy": float(dy), "pen_state": "draw"} for dx, dy in deltas.tolist()]
    return [
        {"dx": float(dx), "dy": float(dy), "pen_state": REGRESSION_ID_TO_PEN.get(int(pen), "draw")}
        for (dx, dy), pen in zip(deltas.tolist(), pens.tolist())
    ]
...
def teacher_forced_sample(args, model, state, device):
    ...
    pred = decode_gmm(out, strategy=args.decode_strategy)[0].cpu()
    pred_pen = out["pen_logits"][0].argmax(dim=-1).cpu()
    coords_abs = denorm_xy(sample["decoder_coords"], canvas_size)
    target = sample["target_dxdy"]
    target_pen = sample["target_pen"]
    ...
    payload = {
        ...
        "target_strokes": strokes_from_deltas(target[mask], target_pen[mask]),
        "pred_strokes": strokes_from_deltas(pred[mask], pred_pen[mask]),
    }
    return payload
...
def autoreg_sample(args, model, state, device):
    ...
    for i in range(args.max_steps):
        ...
        for j, (px, py) in enumerate(points):
            hist.append([px / canvas_size * 2.0 - 1.0, py / canvas_size * 2.0 - 1.0, j / max(model.decoder.cfg.max_len - 1, 1)])
        hist_tensor = torch.tensor(hist, dtype=torch.float32, device=device).unsqueeze(0)
        mask = torch.ones(1, hist_tensor.size(1), dtype=torch.bool, device=device)
        out = model(prompts=[args.prompt], decoder_coords=hist_tensor, target_mask=mask)
        pred = decode_gmm(out, strategy=args.decode_strategy)[0, -1].cpu()
        pen_id = int(out["pen_logits"][0, -1].argmax(dim=-1).item())
        pen_state = REGRESSION_ID_TO_PEN.get(pen_id, "draw")
        dx, dy = pred.tolist()
        deltas.append([dx, dy, pen_state])
        x += dx
        y += dy
        points.append([x, y])
        if pen_state == "end":
    ...
    return {
        ...
        "strokes": [{"dx": float(dx), "dy": float(dy), "pen_state": pen_state} for dx, dy, pen_state in deltas],
    }


def main() -> None:
    parser = argparse.ArgumentParser(description="Visualize continuous GMM regression decoder predictions.")
    parser.add_argument("--checkpoint", type=str, default="runs/stroke_gmm_regression_scale8/checkpoint.pt")
    ...
    parser.add_argument("--decode-strategy", choices=["top", "mean"], default="top")
    parser.add_argument("--png", type=str, default="viz_output/gmm_regression_sample.png")

...
from .gmm_regression_model import (
    GMMCoordDecoderConfig,
    TextConditionedGMMCoordModel,
    decode_gmm,
    gmm_nll,
    gmm_summary_metrics,
)
...
def compute_loss(batch: dict, out: dict, args: argparse.Namespace) -> tuple[torch.Tensor, dict[str, float]]:
    ...
    nll = gmm_nll(out, target, mask)
    pen_loss = F.cross_entropy(
        out["pen_logits"].reshape(-1, out["pen_logits"].size(-1)),
        batch["target_pen"].reshape(-1),
        ignore_index=-100,
    )
    pred = decode_gmm(out, strategy=args.decode_strategy)
    pred_xy = torch.cumsum(pred * mask.unsqueeze(-1).to(pred.dtype), dim=1)
    true_xy = torch.cumsum(target * mask.unsqueeze(-1).to(target.dtype), dim=1)
    xy_loss = masked_mae(pred_xy, true_xy, mask)
    end_loss = torch.nn.functional.smooth_l1_loss(last_valid_values(pred_xy, mask), last_valid_values(true_xy, mask))
    loss = nll + args.pen_loss_weight * pen_loss + args.xy_loss_weight * xy_loss + args.end_loss_weight * end_loss
    with torch.no_grad():
        valid_pen = batch["target_pen"] != -100
        pred_pen = out["pen_logits"].argmax(dim=-1)
        pen_acc = (pred_pen[valid_pen] == batch["target_pen"][valid_pen]).float().mean()
    metrics = {
        ...
        "top_delta_mae": float(masked_mae(pred, target, mask).item()),
        "top_xy_mae": float(masked_mae(pred_xy, true_xy, mask).item()),
    }
    metrics.update(gmm_summary_metrics(out, mask))
    return loss, metrics


def save_checkpoint(output_dir: Path, model: TextConditionedGMMCoordModel, args: argparse.Namespace, epoch: int) -> None:
...
def train_one_epoch(model, loader, optimizer, device, args, epoch, step_log_path: Path):
    ...
    for step, batch in enumerate(loader, start=1):
        ...
        if step % args.log_every == 0:
            print(
                f"epoch={epoch} step={step}/{len(loader)} loss={metrics['loss']:.5f} nll={metrics['nll']:.5f} "
                f"pen_acc={metrics['pen_acc']:.3f} top_mae={metrics['top_delta_mae']:.4f} "
                f"xy_mae={metrics['top_xy_mae']:.4f} sigma={metrics['avg_sigma']:.4f}",
                flush=True,
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train text-conditioned GMM coordinate-to-dxdy decoder.")
    parser.add_argument("--data", type=str, default="generated_data/polar_balanced/polar_balanced_scale8_train.jsonl")
    parser.add_argument("--output-dir", type=str, default="runs/stroke_gmm_regression_scale8")
    ...
    parser.add_argument("--draw-only", action="store_true", help="Skip move steps. Default keeps draw/move/end for 3-state pen training.")
    parser.add_argument("--pen-loss-weight", type=float, default=1.0)
    parser.add_argument("--xy-loss-weight", type=float, default=0.2)
    parser.add_argument("--end-loss-weight", type=float, default=0.1)
    parser.add_argument("--noise-std", type=float, default=0.0)
    parser.add_argument("--decode-strategy", choices=["top", "mean"], default="mean")
    ...
    model = TextConditionedGMMCoordModel(cfg, text_encoder_dir=args.text_encoder_dir, max_text_len=args.max_text_len).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            ...
            f"val_nll={val_metrics['nll']:.5f} val_top_mae={val_metrics['top_delta_mae']:.4f} "
            f"val_xy_mae={val_metrics['top_xy_mae']:.4f} val_pen_acc={val_metrics['pen_acc']:.3f} "
            f"sigma={val_metrics['avg_sigma']:.4f}",
            flush=True,
        )
        append_jsonl(
            epoch_log_path,
